...
class WorthDB {
	...
	public function credentialsValid($email, $password) {
		...
		$query = "SELECT * FROM users WHERE email = '" . mysqli_real_escape_string($this->link, $email) . "' AND password = '" . mysqli_real_escape_string($this->link, $password) . "';";
	...
	public function getTransactions($accountName) {
		$query = "SELECT accountName, merchant, amount, `date`, category FROM transactions WHERE email='" . mysqli_real_escape_string($this->link, $this->email) . "' AND accountName = '" . mysqli_real_escape_string($this->link, $accountName) . "' ORDER BY `date`;";
		$result = $this->getQueryResult($query);
		$transactions = array();
		$i = 0;
	...
	public function getAccounts() {
		...
		$query = "SELECT accountName FROM accounts WHERE email = '" . mysqli_real_escape_string($this->link, $this->email) . "' ORDER BY accountName;";
		$result = $this->getQueryResult($query);
		$accounts = array();
		$i = 0;
		if ($result == null) {
	...
	public function deleteAccount($accountName) {
		$query = "DELETE FROM accounts WHERE email = '" . mysqli_real_escape_string($this->link, $this->email) . "' AND accountName = '" . mysqli_real_escape_string($this->link, $accountName) . "';";
		$result = $this->getQueryResult($query);
		$query = "DELETE FROM transactions WHERE email = '" . mysqli_real_escape_string($this->link, $this->email) . "' AND accountName = '" . mysqli_real_escape_string($this->link, $accountName) . "';";
		$result = $this->getQueryResult($query);
		return $result;
	}

	public function getTransactionsForGraph($startDate, $endDate, $accountName) {
		$query = 'SELECT `date`, amount FROM transactions WHERE email = "' . mysqli_real_escape_string($this->link, $this->email) . '" AND accountName = "' . mysqli_real_escape_string($this->link, $accountName) . '" AND `date` >= "' . mysqli_real_escape_string($this->link, $startDate) . '" AND `date` <= "' . mysqli_real_escape_string($this->link, $endDate) . ' 23:59:59" ORDER BY `date` ASC;';
		$result = $this->getQueryResult($query);
		if ($result == null) {
			return array();
		}
	...
	public function getTotalAssets($startDate, $endDate) {
		$query = 'SELECT `date`, amount FROM transactions WHERE email = "' . mysqli_real_escape_string($this->link, $this->email) . '" AND asset = 1 AND `date` >= "' . mysqli_real_escape_string($this->link, $startDate) . '" AND `date` <= "' . mysqli_real_escape_string($this->link, $endDate) . ' 23:59:59" ORDER BY `date` ASC;';
		$result = $this->getQueryResult($query);
	...
	public function getTotalLiabilities($startDate, $endDate) {
		$query = 'SELECT `date`, amount FROM transactions WHERE email = "' . mysqli_real_escape_string($this->link, $this->email) . '" AND asset = 0 AND `date` >= "' . mysqli_real_escape_string($this->link, $startDate) . '" AND `date` <= "' . mysqli_real_escape_string($this->link, $endDate) . ' 23:59:59" ORDER BY `date` ASC;';
		$result = $this->getQueryResult($query);
		$totalAssets = array(); // map from `date` (as string) to number
		$sum = 0;
		if ($result == null) {
	...
	public function getNetWorths($startDate, $endDate) {
		$query = 'SELECT `date`, amount FROM transactions WHERE email = "' . mysqli_real_escape_string($this->link, $this->email) . '" AND `date` >= "' . mysqli_real_escape_string($this->link, $startDate) . '" AND `date` <= "' . mysqli_real_escape_string($this->link, $endDate) . ' 23:59:59" ORDER BY `date` ASC;';
		$result = $this->getQueryResult($query);
	...
	public function getTotalExpenditureOfCategory($category) {
		$query = 'SELECT budget FROM budgets WHERE email="' . mysqli_real_escape_string($this->link, $this->email) . '" AND category = "' . mysqli_real_escape_string($this->link, $category) . '";';
		$result = $this->getQueryResult($query);
		...
		while ($row = $result->fetch_assoc()) {
			...
			$query = 'SELECT amount FROM transactions WHERE email = "' . mysqli_real_escape_string($this->link, $this->email) . '" AND category = "' . mysqli_real_escape_string($this->link, $category) . '" AND date >= DATE_FORMAT(NOW(), "%Y-%m-01") AND date <= NOW();';
	...
	public function setBudget($category, $amount) {
		...
		$query = 'DELETE FROM budgets WHERE email="' . mysqli_real_escape_string($this->link, $this->email) . '" AND category = "' . mysqli_real_escape_string($this->link, $category) . '";';
		$result = $this->getQueryResult($query);
